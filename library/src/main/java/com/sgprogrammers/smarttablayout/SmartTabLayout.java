 ...
 */
package com.sgprogrammers.smarttablayout;
...
public class SmartTabLayout extends HorizontalScrollView {
  ...
  private ColorStateList tabViewUnselectedTextColors;
  private boolean disableTabHighlighting;
  ...
  public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    ColorStateList unselectedTextColors;
    boolean disableTabHighlighting = false;
    ...
    unselectedTextColors = a.getColorStateList(
            R.styleable.stl_SmartTabLayout_stl_defaultUnselectedTabTextColor);
    disableTabHighlighting = a.getBoolean(
            R.styleable.stl_SmartTabLayout_stl_disableTabHighlighting, disableTabHighlighting);
    ...
    this.tabViewUnselectedTextColors = (unselectedTextColors != null)
            ? unselectedTextColors
            : ColorStateList.valueOf(TAB_VIEW_TEXT_COLOR);
    this.disableTabHighlighting = disableTabHighlighting;
   ...
   *
   * @param interpolator {@link com.sgprogrammers.smarttablayout.SmartTabIndicationInterpolator}
   */
  ...
  private void scrollToTab(int tabIndex, float positionOffset) {
    ...
    for (int index = 0; index<adapter.getCount();index++) {
      TextView selectedTab = (TextView)tabStrip.getChildAt(index);

      if (selectedTab instanceof TextView) {
        if (tabIndex == index) {
          selectedTab.setTextColor(tabViewTextColors);
        }
        else {
          selectedTab.setTextColor(tabViewUnselectedTextColors);
        }

        if (disableTabHighlighting) {
